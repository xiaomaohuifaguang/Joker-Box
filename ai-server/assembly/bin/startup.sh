...
if [ -s "${PID_FILE}" ]; then
  ...
  ## 启动
  echo "Starting ..."
  nohup java -Dfile.encoding=UTF-8 -Dspring.config.import=file:$SERVER_FOLDER/config/ -jar $SERVER_BIN_FOLDER/ai-server-1.0-SNAPSHOT.jar > $SERVER_FOLDER/log/server.log 2>&1 &
